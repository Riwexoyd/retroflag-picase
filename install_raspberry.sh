...
SourcePath=https://raw.githubusercontent.com/Riwexoyd/retroflag-picase/raspberry_pi_os
...
script=/opt/RetroFlag/SafeShutdown_raspberry.py
wget -O $script "$SourcePath/SafeShutdown_raspberry.py"
...
if grep -q "sudo python $script &" "$RC";
	then
		...
		sed -i -e "s/^exit 0/sudo python \/opt\/RetroFlag\/SafeShutdown_raspberry.py \&\n&/g" "$RC"
